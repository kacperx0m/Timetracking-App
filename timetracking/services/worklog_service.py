...
class WorklogService:
    ...
    @staticmethod
    def calculate_planned_worktime(single_schedule):
        ...
        planned_worktime = time_end - time_start
        return planned_worktime / timedelta(hours=1)

    @staticmethod
    def check_if_late(single_schedule, daily_events):
        THRESHOLD_MINUTES = 5
        check_in_event = daily_events.filter(event_type="CHECK_IN").first()
        ...
        return minutes_late if minutes_late >= THRESHOLD_MINUTES else 0.0
    ...
    @staticmethod
    def create_daily_worklog(employee, single_schedule, daily_events):
        print("creating daily worklog")
        anomalies = []
        absent = False
        planned_hours = int(WorklogService.calculate_planned_worktime(single_schedule))
        worked_hours = 0.0
        break_minutes = 0.0
        minutes_late = 0.0
        is_leave = False
